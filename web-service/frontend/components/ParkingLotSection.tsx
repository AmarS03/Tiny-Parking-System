...
import SplineComponent, { SplineComponentHandle } from "@/components/ParkingLot/SplineComponent";
...
export function ParkingLotSection({
    ...
    const prevSpotsRef = useRef<parkingSpot[]>([]);
    const splineRef = useRef<SplineComponentHandle>(null);

    useEffect(() => {
        if (prevSpotsRef.current.length > 0) {
            spotsData.forEach((currentSpot) => {
                const previousSpot = prevSpotsRef.current.find(s => s.id === currentSpot.id);
        ...
        prevSpotsRef.current = spotsData;
    }, [spotsData]);
                ...
                >
                    <ResizablePanel defaultSize={isSmallScreen ? 0 : 60}>
                        <div className="w-full h-124">
                            <SplineComponent ref={splineRef} />
                        </div>
                    </ResizablePanel>
                    <ResizableHandle />
                    <ResizablePanel defaultSize={isSmallScreen ? 100 : 40}>
                        <div
                            className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-2 xl:grid-cols-5 gap-4 w-full h-full p-4"
                            style={{ transform: "rotateX(20deg) scale(0.95)" }}
                        >
                            {spotsData.map((spot) => (
                                <div
                                    key={spot.id}
                                ...
                                >
                                    ...
                                    {spot.isOccupied ? (
                                        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                                            <span className="-rotate-12 bg-white/90 dark:bg-black/80 px-2 py-1 rounded border shadow-sm font-mono text-xs sm:text-sm font-bold tracking-wider text-slate-800 dark:text-slate-200 transform translate-y-4">
                                                {spot.occupiedBy}
                                            </span>
                                        </div>
                                    ) : (
